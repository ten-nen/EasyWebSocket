...
*/
    ...
    initFlashWebSocket();
    var that = {
        swfLocation: 'WebSocketMain.swf',
        debug: false,
        start: function (uri) {
            window.WEB_SOCKET_DEBUG = that.debug;
            window.WEB_SOCKET_SWF_LOCATION = that.swfLocation;
...
replace(/(?:^|:|,)(?:\s*\[)+/g, '')))
                ...
                try {
                    ...
                    if (!json.method || !that[json.method])
                        return;
                    ...
                    if (!isArray(json.data)) {
                        that[json.method](json.data);
                        return;
                    }
                    that[json.method].apply(that, json.data);
                } catch (e) { }
            }
        },
        send: function (method) {
            ...
            ws.send(JSON.stringify({ method: arguments[0], data: data }));
        },
        close: function () {
            ws.close();
        },
